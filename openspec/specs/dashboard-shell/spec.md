...
The logged-in area of the site: one frame — identity, sign-out and a rail of the views
this viewer may use — with the selected view beside it, addressed by path segment so it
is linkable and survives a reload. Growth (owner only) and practice (any signed-in user)
are the two views today; the registry in `views.tsx` is the one place that lists them.
