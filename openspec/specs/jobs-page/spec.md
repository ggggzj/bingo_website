...
The public job feed on this site: roles from employers with a record of sponsoring H-1B
visas, with the certified filing count and years shown on every listing.

A feed, deliberately, not a search engine. The postings come from a limited set of employer
job boards, so a search box promising to find any company could not keep that promise. The
page opens on the list and states its own coverage.

Read-only and identity-free. No session is read, nothing is written, and opening it twice
returns the same page. Its data comes from the extension's API through a server-side proxy
holding a shared secret that no browser ever sees.
