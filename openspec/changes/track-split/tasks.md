...
- [x] 1.1 Draft the pattern→multiplier table for the `ai-engineer` track
      ...
      rationale.
- [x] 1.2 **STOP — owner approves the table** (adjust boosts, add drops);
      record the approval in the research file.

## 2. Engine

- [x] 2.1 Add `src/trackWeights.ts` (approved table + `trackFactor(problem,
...
- [x] 3.1 Add `active_track` (text, default `sde`) to `coach_config`,
...
- [x] 3.2 Add the SDE / AI Engineer toggle to the settings panel with the
