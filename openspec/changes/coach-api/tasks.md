...
- [x] 1.1 Add `coach_api_tokens` table to `lib/db/src/schema/coach.ts`
...
- [x] 1.2 Add the `coach` tag and all nine operations (plan, solved, grade,
...
- [x] 2.1 Add `lib/auth/coach.ts` (`coachEmails`, `isCoachUser`) mirroring
      owner.ts semantics; verify with unit tests: unset var means nobody,
      trimming and case-insensitivity, blank entries dropped.
- [x] 2.2 Extend the `CoachStore` seam with token operations and add
...
- [x] 2.3 Implement `POST /coach/token` (issue, revoking predecessors,
...
- [x] 3.1 Define `CoachStore` (problems, review get/upsert+event, day-log
      get/put, config get/put, tokens) and implement `memory-store`;
      verify the interface compiles and memory store passes a shared
      contract test file.
- [x] 3.2 Implement `drizzle-store` including the grade transaction
...
- [x] 4.1 Implement `GET /coach/plan` with freeze-on-first-call using the
...
- [x] 4.2 Implement `POST /coach/solved` and `POST /coach/grade` over the
...
- [x] 4.3 Implement `GET /coach/forecast`, `GET /coach/log` (statuses,
...
- [x] 4.4 Mount the coach router in `routes/index.ts`, add
...
- [x] 5.2 End-to-end smoke against a local Postgres: seed problems, create
